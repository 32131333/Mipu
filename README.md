...
> [!IMPORTANT]
> Всем привет! Сегодня 16 мая 2026 года, и я хочу заявить, что *Mipu* **закрыт на неопределенный срок** из-за того, что я ПРОСТО ТАКОЙ ЖЕ МУСОР ненужный, как и это веб-приложение. Я просто сжигаю ресурсы. Я НИКТО, я ошибка эволюции. За годы я надеялся хоть на что-то, но в итоге так был никчемным, так и остался. В скором постараюсь открыть бэкенд. Если вам интересно хоть что-то, и хотите со мной связаться, можете мне написать в [Telegram](https://t.me/d32131333?direct). Не продаю
...
```
{
 ...
 "frontend_disable_auto_build": false
}
```
...
`frontend_disable_auto_build` может быть полезен в местах, где сервер часто перезапускается, или `bundle.js` уже был сгенерирован. Но в таком случае придется самим билдить через команду `npm run build`
